...
webdriver_path = '/usr/bin/chromedriver' if platform.system() != 'Windows' else 'chromedriver.exe'
...
def __scrape(url, bar_position=0):
    ...
    while True:
        ...
        """
        # Stop after scraping 5 jobs
        if last_scraped_job >= 5:
            pbar.close()
            break
        try:
...
if __name__ == '__main__':
	...
	df: pd.DataFrame = get_listings_from(urls).dropna(axis=0, subset='full_url')
